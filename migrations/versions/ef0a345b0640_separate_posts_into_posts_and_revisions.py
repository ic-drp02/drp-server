...
def upgrade():
    ...
    op.create_table('post_revisions',
                    ...
                    sa.ForeignKeyConstraint(
                        ['post_id'], ['posts_new.id'], ondelete="SET NULL"),
                    sa.PrimaryKeyConstraint('id')
                    )
    ...
    op.create_foreign_key(None, 'posts', 'post_revisions',
                          ['latest_rev_id'], ['id'], ondelete="SET NULL")
...
def downgrade():
    ...
    op.add_column('posts', sa.Column('created_at', postgresql.TIMESTAMP(
        timezone=True), server_default=sa.text('now()'), autoincrement=False,
        nullable=False))
    op.add_column('posts', sa.Column('title', sa.VARCHAR(
        length=120), autoincrement=False, nullable=False))
    op.add_column('posts', sa.Column('summary', sa.VARCHAR(
        length=200), autoincrement=False, nullable=True))
    op.add_column('posts', sa.Column('is_current', sa.BOOLEAN(),
                                     autoincrement=False, nullable=True))
    op.add_column('posts', sa.Column('content', sa.TEXT(),
                                     autoincrement=False, nullable=True))
    op.add_column('posts', sa.Column('post_id', sa.INTEGER(),
                                     server_default=sa.text(
        "nextval('post_id_seq'::regclass)"), autoincrement=True,
        nullable=True))
    op.drop_constraint('posts_latest_rev_id_fkey', 'posts', type_='foreignkey')
    op.create_index('unique_current_post_id', 'posts', [
                    'post_id', 'is_current'], unique=True)
    op.drop_column('posts', 'latest_rev_id')
    op.add_column('files', sa.Column('post_id', sa.INTEGER(),
                                     autoincrement=False, nullable=True))
    op.drop_constraint(None, 'files', type_='foreignkey')
    op.create_foreign_key('files_post_id_fkey', 'files',
                          'posts', ['post_id'], ['id'])
    op.drop_column('files', 'post_rev_id')
    op.create_table('post_tag',
                    ...
                    sa.PrimaryKeyConstraint(
                        'post_id', 'tag_id', name='post_tag_pkey')
                    )
    op.drop_table('post_rev_tag')
